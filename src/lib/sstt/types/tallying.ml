...
module Make(VS:VarSettings) = struct
  ...
  let norm, norm_field =
    ...
    and norm_record (ps, ns) =
      ...
      norm_tuple_gen ~any:Ty.F.any ~conj:Ty.F.conj
        ~diff:Ty.F.diff ~norm:norm_field n line
  ...
  let propagate cs =
    ...
    let rec aux (prev,prev') ((cs,cs') : CS'.t) =
      let retry_with css =
        ...
        css |> CSS.to_list |> List.map (aux CS'.any) |> CSS.disj
      in
      match cs,cs' with
  ...
  let tally cs =
    ...
    let mcss = ncss |> CSS.to_list |> List.map propagate |> CSS.disj in
