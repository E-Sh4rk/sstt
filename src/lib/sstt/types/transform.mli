...
val simplify_vdescr : ?normalize:(Ty.t -> Ty.t) -> VDescr.t -> VDescr.t
(** [simplify_vdescr ?normalize vd] returns a [VDescr.t] equivalent to [vd]
    but where atoms of all top-level components have been merged together when possible.
    If [?normalize] is provided, atoms of DNFs that become redundant
    when [normalize] is applied to the type are filtered out.
*)
