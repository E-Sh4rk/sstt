...
module type RecordAtom = sig
  ...
  (* type oty = node * bool *)
  (** An optional type (see {!Sstt.Ty.F}). *)

  type t = { bindings : FieldTy.t LabelMap.t;(** mapping from labels to optional types *)
...
module type Records = sig
  ...
  *)
  ...
  val dnf_line_to_tuple : (Atom.t list * Atom.t list * bool) -> (Atom.FieldTy.t list list * Atom.FieldTy.t list list * bool) * int
  ...
  *)
  include OptComponent with type t := t
                        and type node := node
                        and module type Atom' := (RecordAtom' with type node := node and module FieldTy = Atom.FieldTy)
