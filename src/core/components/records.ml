...
module Tail = struct
  ...
  let equal t1 t2 =
    ...
    | Open, Open -> true
    | Closed, Closed -> true
    | RowVar v1, RowVar v2 -> Var.equal v1 v2
    | _ -> false
  ...
  let compare t1 t2 =
    ...
    | Open, Open -> 0
    | Closed, Closed -> 0
    | RowVar v1, RowVar v2 -> Var.compare v1 v2
    | Open, _ -> -1
    | _, Open -> 1
    | Closed, _ -> -1
    | _, Closed -> 1
...
module Atom(N:Node) = struct
  ...
  let simplify t =
    ...
    | _ -> { t with bindings = LabelMap.filter not_any t.bindings }
  let equal t1 t2 =
...
module Make(N:Node) = struct
  ...
  module DnfAtom' = struct
    ...
    let to_t' (a,b) =
      ...
      let not_binding (l,on) =
        { bindings=LabelMap.singleton l (ON.neg on) ; tail=Open ; required=None }
      in
      if b then
  ...
  let dnf' t = dnf t |> Dnf'.from_dnf
                 ({ Atom'.bindings=LabelMap.empty ; tail=Open ; required=None })
