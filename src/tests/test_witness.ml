...
let%expect_test _ = 
  ...
  [%expect {|
    ...
    ('A, 'A) \ ('B, 'A) : 42, 42 with subst [ 'A : any ; 'B : empty ]
    'a, ~'a : 41, 42 with subst [ 'a : ~42 ]
    ~'a & (0..4) | (0..1) : 0 with subst [ 'a : empty ]
    'a & (0..4) : 0 with subst [ 'a : any ]
    'a & ~'b & ~(0..3) : -1 with subst [ 'a : any ; 'b : empty ]
    ...
    'A & ~'B & ~int : " a " with subst [ 'A : any ; 'B : empty ]
    ~'B \ int | 'A & ~'B & ~(int \ 42) : " a " with subst [ 'A : empty ; 'B : ~(a | int) ]
    |}]